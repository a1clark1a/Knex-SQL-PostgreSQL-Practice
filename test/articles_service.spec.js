...
describe(`Articles service object`, () => {
  ...
  //before starting the test make a knex instance called db
  before(() => {
    db = knex({
      client: "pg",
  ...
  //so everytime we test before it tests we clean the table with truncate
  before(() => db("blogful_articles").truncate());

  //to avoid testleak every test should clean up after itself
  afterEach(() => db("blogful_articles").truncate());

  //after the test disconnect from the database
  after(() => db.destroy());

  //context is the same as describes
  context(`given 'blogful_articles' has data`, () => {
    ...
    it(`updateArticle() updates an aritcle from the 'blogful_articles' table`, () => {
    ...
    it(`insertArticle() inserts a new article and resolves the new article with an 'id'`, () => {
      ...
      return ArticlesService.insertArticle(db, newArticle).then(actual => {
        expect(actual).to.eql({
          ...
          //if timezone are different use date_published: new Date(newArticle.date_published),
